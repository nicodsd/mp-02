    ...
    const formik = useFormik({
        ...
        onSubmit: async (values, { resetForm }) => {
            try {
                ...
                await fetch(`${URI}/auth/reset-password`, {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json",
                    },
                    body: JSON.stringify({ email: values.email, token: token, password: values.password }),
                    credentials: "include",
                });
                    ...
                    <div className="w-full py-8 pt-14 relative mt-2">
                        ...
                        <form onSubmit={formik.handleSubmit} className="space-y-4 px-3">
                            ...
                            <div className="flex flex-col w-full mb-4">
                                <label htmlFor="password" className="text-md text-gray-700">Nueva contraseña</label>
                                <input
                                    id="password"
                                    name="password"
                                    type="password"
                            ...
                            <div className="relative">
                                <label htmlFor="confirmPassword" className="text-md text-gray-700">Confirmar contraseña</label>
                                <input
                                    id="confirmPassword"
                            ...
                            <button
                                ...
                                className={`w-full bg-primary ${status === "loading" ? "cursor-not-allowed opacity-70" : "active:scale-90 cursor-pointer"} text-white font-bold py-4 px-3 rounded-lg uppercase tracking-wide transition-all`}
                            >
                        ...
                        {/* Feedback */}
                        {status === "success" && (
                            <p className="text-green-600 text-sm mt-3 text-center">Contraseña restablecida correctamente ✅</p>
                        )}
                        {status === "error" && (
                            <p className="text-red-600 text-sm mt-3 text-center">Hubo un error al restablecer la contraseña ❌</p>
                        )}
