...
export default function NavBar({
  ...
  const navVariants: Record<number, JSX.Element> = {
    0: (
      <div
        className={`flex items-center justify-between w-full px-5 md:px-10 h-full ${background ? "" : "bg-transparent"}`}
      >
        <div className="flex items-center flex-col w-full h-full justify-end pb-15 text-white">
          {cookie && user ? (
            <div
              className={`flex flex-col items-center ${cookie && user ? "gap-4" : "gap-2"} ${background ? "drop-shadow-xl drop-shadow-black/30" : ""}`}
            >
              <Link className="relative" href="/panel-de-usuario">
                ...
                />
                <div className="text-black absolute -bottom-2 font-bold bg-white rounded-lg left-1/2 transform w-20 -translate-x-1/2 text-xs px-2 py-1">
                  Editar Perfil
                </div>
              </Link>
              <div className="flex flex-col items-center">
                <span
                  className={`text-2xl uppercase font-bold pb-1 ${background ? "text-white" : "text-gray-900"}`}
                >
                  {name}
                </span>
                <p
                  className={`text-sm ${background ? "text-white" : "text-gray-900"}`}
                >
                  {description}
                </p>
                <div
                  className={`flex flex-col items-center ${background ? "text-gray-200" : "text-gray-900"}`}
                >
                  {address && (
                    <p className="text-xs flex items-center gap-1 font-light">
                      <FaMapMarkerAlt /> {address}
                    </p>
                  )}
                  {phone && (
                    <p className="text-xs flex items-center gap-1 font-light">
                      <FaPhone className="rotate-180" /> {phone}
                    </p>
                  )}
                </div>
              </div>
            ...
            <div
              className={`flex flex-col items-center gap-3 ${background ? "drop-shadow-xl drop-shadow-black/30" : ""}`}
            >
              <div
                className={`relative rounded-full ${background ? "bg-white" : ""}`}
              >
                <Image
                  width={100}
                  height={100}
    ...
    <>
      <div className="flex flex-col w-full items-center">
        ...
        {background ? (
          ...
          <div
            className={`flex justify-end w-full bg-radial from-transparent ${
              state !== 1 && state !== 2
