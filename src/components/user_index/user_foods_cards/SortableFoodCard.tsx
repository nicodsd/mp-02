    ...
    return (
        <div
            className={`flex w-full overflow-hidden 
                ...
                ${context
                    ? `border-r ${template?.border} md:pr-6 px-2 rounded-lg py-2 h-full items-center`
        ...
        >
            ...
            <div className="flex flex-col justify-between pl-2 w-full h-full">
                <div className="menu-card__content text-left">
                    <h2
                        className={`font-semibold ${template?.textColor} text-base md:h-fit ${context ? "leading-4" : "h-4 md:text-2xl"}`}
                    >
                        {food.name}
                    </h2>
                    <p
                ...
                >
                    {food.is_promo && !context && (
                        <div className="flex flex-col items-end">
                            ...
                            <div className="flex items-center gap-1">
                                <span className={`font-black ${template?.textColor} oldstyle-nums text-lg md:text-xl`}>
                    ...
                    {!food.is_promo && (
                        <div className="flex items-center gap-1">
                            <span className={`font-bold ${template?.textColor} oldstyle-nums text-lg md:text-xl`}>
                                {formatearPrecio(food.price)}
                            </span>
                        </div>
                    )}
