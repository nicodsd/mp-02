...
const UserSettings = ({ user, token }: { user: any; token: string }) => {
  ...
  // Estados de control de edición (individuales)
  const [editStates, setEditStates] = useState({
    name: false,
    description: false,
    location: false,
    phone: false,
    photo: false, // Nueva para rastrear si se cambió la imagen
  });

  // El botón de "Guardar" se muestra si CUALQUIER estado es true
  ...
  useEffect(() => {
    ...
    setPhone(user.phone || "");
    // Resetear todos los estados de edición cuando el usuario se actualiza
    setEditStates({
      name: false,
      description: false,
      location: false,
  ...
  // Handler genérico para abrir/cerrar ediciones sin chocar entre sí
  ...
  const imageCapture = (selectedFile: File) => {
    ...
    setEditStates((prev) => ({ ...prev, photo: true })); // Activamos modo edición al cambiar foto
  };
