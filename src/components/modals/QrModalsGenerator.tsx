...
export default function QrModalsGenerator({
  ...
  const handleDownload = () => {
    ...
    // Aplicamos el color de fondo de la plantilla
    ...
    if (logoUrl) {
      ...
      img.onload = () => {
        ...
        // El fondo del logo usa el color de fondo de la plantilla para el borde
        ctx.fillStyle = selectedTemplate.bg;
        ctx.beginPath();
  ...
  const finalizeDownload = (ctx: CanvasRenderingContext2D, text: string) => {
    ...
    // El texto usa el color de contenido (Foreground) de la plantilla
      ...
      {isOpen && (
        <div className={`fixed ${template?.backgroundColor2} backdrop-blur-sm inset-0 flex items-center justify-center z-100`}>
          <div className={`p-6 text-center w-[95%] max-w-96 animate-in fade-in zoom-in duration-200`}>
            ...
            <div
              className={`relative flex justify-center mb-8 p-6 rounded-2xl border ${template?.border} transition-colors duration-300`}
